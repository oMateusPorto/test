...
export default function UserMaintenance() {
    ...
    const { openStage, getMaintenanceId, deleteStages } = useContext(StageContext) as StageContextType
    ...
    function stageOfMaintenance(id: string, status: boolean) {
        getMaintenanceId(id, status)
        openStage(id)
        navigate('/Stages')
    }
                                    ...
                                    <div className="d-flex card-header">
                                        ...
                                        <span>
                                            ...
                                            <i
                                                ...
                                                style={{ cursor: 'pointer' }}
                                                onClick={() => [deleteMaintenance(item.id), deleteStages(item.id)]}
                                            ></i>
                                        </span>
                                    </div>
                                    <div className="card-body">
                                        <p>{` Data de cadastro: ${item.createdAt}`}</p>
                                        <p>{` Status: ${item.status ? 'Manutenção finalizada' : 'Manutenção em execução'}`}</p>
                                        <div className=" d-flex pt-2 m-0 border-top justify-content-end">
                                            <div className='d-flex justify-content-end'>
                                                <button
                                                    className='btn btn-sm btn-outline-primary'
                                                    onClick={() => stageOfMaintenance(item.id, item.status)}
                                                >
                                                    Etapas
                                                </button>
                                            </div>
